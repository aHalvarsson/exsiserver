...
const { logger, helper } = require('./modules');

const reportTemplate = {
   header: function ({ heightPx, oddRowColor }) {
      return `<!DOCTYPE html>
<html lang="en">
	<head>
		<meta charset="UTF-8" />
		...
		<style>
			* {
			...
			.mainBodyTable #table-body .odd td {
				...
				background-color: ${oddRowColor} !important;
			}
	...
	<body>
      ...
      path.join(__dirname, 'tableHeader.jpg')
   ).toString(
      'base64'
   ...
   dataRows: function ({
      ...
      return `<tr${oddOrNot ? '' : ' class="odd"'}>
							...
							<td class="right-align" width="18%" class="tdRight">${helper.formNum(debet)}</td>
							<td class="right-align" width="18%">${helper.formNum(kredit)}</td>
						</tr>`;
   },
							...
							<div class="relativeContainer bottom">
								<table class="titleTable bottom">
									<tr>
										...
										<td width="24.5%" class="tdRight right-align">${helper.formNum(sumDebet)}</td>
										<td class="right-align">${helper.formNum(sumKredit)}</td>
									</tr>
								</table>
								<img src="data:image/jpeg;base64,${readFileSync(
                           path.join(__dirname, 'tableHeaderSmall.jpg')
   ).toString(
      'base64'
   ...
   headTemplate: function ({
      ...
      try {
		...
		<style>
			* {
			...
			.title-bar {
				...
				margin: 0 0 10px;
				position: relative;
				border-bottom: solid 2px ${titleColor} !important;
		...
		<div class="pageHeader">
         ...
         logger.error({
            message: `Head template error: ${error.message}`,
