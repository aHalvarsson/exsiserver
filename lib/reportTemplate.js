...
<html lang="en">
	...
	dataRows: function ({ oddOrNot, account, costCentre, description, debet, kredit }) {
		return `<tr${oddOrNot ? '' : ' class="odd"'}>
							...
							<td class="right-align" width="18%" class="tdRight">${debet ? debet.toFixed(2) : ''}</td>
							<td class="right-align" width="18%">${kredit ? kredit.toFixed(2) : ''}</td>
						</tr>`;
	},
							...
							<div class="relativeContainer bottom">
								<table class="titleTable bottom">
									<tr>
										...
										<td width="24.5%" class="tdRight">${sumDebet ? sumDebet.toFixed(2) : ''}</td>
										<td>${sumKredit ? sumKredit.toFixed(2) : ''}</td>
									</tr>
								</table>
