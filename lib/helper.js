...
const helper = {
  ...
  createTask: (data, type, workFunctions) => {
    ...
    task.id = data?.itemId ? data.itemId : uuidv4();
    task.orgNr = data?.orgNr ? data.orgNr : '';
    task.fileType = data?.fileType ? data.fileType : '';
    return task;
  },

  takeATimeout: (ms) => new Promise((resolve) => setTimeout(resolve, ms)),
};
