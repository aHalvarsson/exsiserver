...
async function createPDFBase(dataArray) {
  console.log('createPDFBase', dataArray.length);
  ...
  try {
    ...
    if (!dataArray || !dataArray.length) {
      logger.error({
        message: 'No data provided, array empty or missing!',
      });
      throw new Error('No data provided!');
    }

    for (let i = 0; i < dataArray.length; i++) {
      const data = dataArray[i].extractedData;

      try {
        const stringResults = createPDFhtmlString(data);

        headTemplate = stringResults.headTemplate;
        pageArray = stringResults.pageArray;
        footerTemplate = stringResults.footerTemplate;
        // eslint-disable-next-line no-unused-vars
      } catch (err) {
        logger.error({
          message: 'Data object empty',
        });
        dataArray[i].pdfBytes = '';
        continue;
      }
      ...
      try {
        for (let i = 0; i < pageArray.length; i++) {
          ...
          await page.goto(
            'data:text/html,' + encodeURIComponent(pageArray[i]),
    ...
    console.log(error);
    logger.error({
      message: 'An error occurred while generating the PDFs.',
      metadata: { error },
      codeFile: 'createPDF.js',
    });
    throw error;
...
function createPDFhtmlString(data) {
  try {
    ...
    const headTemplate = reportTemplate.headTemplate({
      docType,
      date,
      orgNr,
      companyName,
    ...
    for (let j = 0; j < numChunks; j++) {
      ...
      for (
        let i = j * chunkSize;
        i < (j + 1) * chunkSize && i < account.length;
        i++
      ) {
