...
class sirLogsALot {
  ...
  _logToConsole(entry) {
    const consoleEntry = `Logger: ${entry.level.charAt(0).toUpperCase() + entry.level.slice(1)} - Labels: ${entry.label} - Message: ${
      entry.message
    }, errorMsg: ${entry.errorMsg}, errorStack: ${entry.errorStack}, company: ${entry.company}, store: ${entry.store}, Metadata: ${entry.metadata}, codefile: ${entry.codefile}, Timestamp: ${entry.timestamp}`;
